 ...
 *   LICENSE: zlib/libpng
 *
 *   raylib-tmx is licensed under an unmodified zlib/libpng license, which is an
...
Color ColorFromTMX(
    uint32_t color); // Convert a Tiled color number to a raylib Color
void DrawTMX(tmx_map *map, int posX, int posY,
             Color tint); // Render the given Tiled map to the screen
void DrawTMXLayers(tmx_map *map, tmx_layer *layers, int posX, int posY,
                   Color tint); // Render all the given map layers to the screen
void DrawTMXLayer(tmx_map *map, tmx_layer *layer, int posX, int posY,
                  Color tint); // Render a single map layer on the screen
void DrawTMXTile(tmx_tile *tile, int posX, int posY,
                 Color tint); // Render the given tile to the screen
...
tmx_map *LoadTMX(const char *fileName) {
  ...
  // tmx_map* map = tmx_load_buffer_path(fileText, TextLength(fileText),
  // fileName); if (!map) {
  //     TraceLog(LOG_ERROR, "TMX: Failed to load TMX file %s", fileName);
  //     return NULL;
  // }
 ...
 * @internal
 */
void DrawTMXPolyline(double offset_x, double offset_y, double **points,
 ...
 * @internal
 */
void DrawTMXPolygon(double offset_x, double offset_y, double **points,
                    int points_count, Color color) {
 ...
 * @internal
 */
void DrawTMXLayerObjects(tmx_map *map, tmx_object_group *objgr, int posX,
                         int posY, Color tint) {
  ...
  while (head) {
    if (head->visible) {
      ...
      case OT_SQUARE:
        ...
        break;
      case OT_POLYGON:
        DrawTMXPolygon(dest.x, dest.y, head->content.shape->points,
                       head->content.shape->points_len, color);
        break;
      case OT_POLYLINE:
        DrawTMXPolyline(dest.x, dest.y, head->content.shape->points,
                        head->content.shape->points_len, color);
        break;
      case OT_ELLIPSE:
 ...
 * @internal
 */
void DrawTMXLayerTiles(tmx_map *map, tmx_layer *layer, int posX, int posY,
                       Color tint) {
    ...
    for (int y = 0; y < map->height; y++) {
      for (int x = 0; x < map->width; x++) {
        ...
        if (map->tiles[gid] != NULL) {
          ...
          DrawTMXTile(map->tiles[gid], posX + x * ts->tile_width,
...
void DrawTMXLayer(tmx_map *map, tmx_layer *layer, int posX, int posY,
  ...
  switch (layer->type) {
  case L_GROUP:
    DrawTMXLayers(map, layer->content.group_head, posX + layer->offsetx,
                  posY + layer->offsety, tint); // recursive call
    break;
  case L_OBJGR:
    DrawTMXLayerObjects(map, layer->content.objgr, posX + layer->offsetx,
                        posY + layer->offsety, tint);
    break;
  case L_IMAGE:
    DrawTMXLayerImage(layer->content.image, posX + layer->offsetx,
                      posY + layer->offsety, tint);
    break;
  case L_LAYER:
    DrawTMXLayerTiles(map, layer, posX + layer->offsetx, posY + layer->offsety,
                      tint);
    break;
  case L_NONE:
    // Nothing.
